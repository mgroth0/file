...
class ZipFiles {
   ...
   * @param zipDirName
   */
  fun zipDirectory(dir: MFile, zipDirName: MFile) {
	...
	try {
	  ...
	  for (filePath in filesListInDir) {
		...
		val ze = ZipEntry(filePath.substring(dir.abspath.length + 1, filePath.length))
		zos.putNextEntry(ze)
   ...
   * @throws IOException
   */
  @Throws(IOException::class) private fun populateFilesList(dir: MFile) {
	val files: Array<MFile> = dir.listFiles()!!
	for (file in files) {
  ...
  companion object {
	 ...
	 * @param zipFileName
	 */
	fun zipSingleFile(file: MFile, zipFileName: String) {
	  ...
	  try {
		...
		fis.close()
		fos.close()
		println(file.canonicalPath + " is zipped to " + zipFileName)
	  } catch (e: IOException) {
		e.printStackTrace()
	  }
	}
