...
import matt.file.ext.j.recursiveLastModified
import matt.file.ext.j.recursiveSize
...
context(CoroutineScope)
fun JioFile.createRecursiveLastModifiedProp(checkInterval: Duration) =
    launchWatchProperty(checkInterval) {
        recursiveLastModified()
    }

context(CoroutineScope)
fun JioFile.createFileSizeProp(checkInterval: Duration) =
    launchWatchProperty(checkInterval) {
        takeIf { exists() }?.size()
    }

context(CoroutineScope)
fun JioFile.createRecursiveFileSizeProp(checkInterval: Duration) =
    launchWatchProperty(checkInterval) {
        takeIf { exists() }?.recursiveSize()
    }

context(CoroutineScope)
fun JioFile.createFileExistsProp() =
    launchFileWatchingProperty(parent!!) {
        exists()
    }
...
private fun <T> CoroutineScope.launchRecursiveDirWatchingProperty(
    ...
    val jioFile = dir.toJioFile()
    check(jioFile.exists())
    check(jioFile.isDirectory)
    val mutex = Mutex()
    val prop = BindableProperty(op())
    val watcher =
        DirectoryWatcher.builder().path(jioFile).listener { event ->
...
private fun <T> CoroutineScope.launchFileWatchingProperty(
    ...
    val prop = BindableProperty(op())
    val jioFile = file.toJioFile()
    check(jioFile.exists())
    check(jioFile.isDirectory)
    val path = jioFile
    val watchService =
        (path as Path).fileSystem.newWatchService() /*eventually, might want to not create a new watch service each time... For now, it is ok though.*/
    val watchKey =
        path.register(
